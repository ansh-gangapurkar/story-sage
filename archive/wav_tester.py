...
import websockets
import uuid
import wave  # Import wave library explicitly here
...
"""
        ...
        try:
            ...
            try:
                ...
                async with websockets.connect(ws_url) as websocket:
                    ...
                    logger.info(f"Generated raw audio data length: {len(audio_data)} bytes for segment output to: {output_file}") # Log raw data length
    ...
    def convert_raw_to_wav(raw_filepath, wav_filepath, sample_rate=8000, channels=1, sample_width=2):
        """Static method to convert raw PCM to WAV with logging."""
        import wave
        logger.info(f"Starting raw to WAV conversion:") # Start log
        logger.info(f"  Raw file: {raw_filepath}")
        logger.info(f"  WAV file: {wav_filepath}")
        logger.info(f"  Parameters: sample_rate={sample_rate}, channels={channels}, sample_width={sample_width}") # Log params
        try:
            with open(raw_filepath, 'rb') as raw_file:
                raw_data = raw_file.read()
                logger.info(f"  Read {len(raw_data)} bytes from raw file.") # Log bytes read
            ...
            logger.info(f"Successfully converted {raw_filepath} to {wav_filepath}") # Success log
        except Exception as e:
            logger.error(f"Error during raw to WAV conversion: {e}") # Error log
